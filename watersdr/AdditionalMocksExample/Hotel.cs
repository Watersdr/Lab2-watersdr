...
namespace Expedia
{
	public class Hotel
	{
        ...
        //Returns the occupant of a room in the hotel
        public String getRoomOccupantWithCheck(int roomNumber)
        {

            if(Database.isRoomOccupantValid(roomNumber)) {
                return Database.getRoomOccupant(roomNumber);
            } else {
                return "check failed";
            }
        ...
        public void updateIfInvalid(int roomNumber, IUpaderService service)
        {
            if (!Database.isRoomOccupantValid(roomNumber))
            {
                service.updateOccupantDB(roomNumber);
            }
        }
